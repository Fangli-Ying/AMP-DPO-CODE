...
def main():
    ...
    parser.add_argument('--batch_size', default=2000, type=int, required=False, help='batch size')
    parser.add_argument('--raw_data_path', default='/Data/Sequence.csv', type=str, required=False, help='pepbase path')
    parser.add_argument('--model_path', default='/AMP_models/ProteoGPT/', type=str, required=False, help='pretrained model path')
    parser.add_argument('--classifier_path', default='/AMP_models/BioToxiPept/best_model.pt', type=str, required=False, help='classifier model path')
    parser.add_argument('--output_path', default='/Data/Sequence_toxinpre.csv', type=str, required=False, help='toxin prediction results output path')
    parser.add_argument('--candidate_pep_path', default='/Data/Sequence_nontoxin.csv', type=str, required=False, help='candidate pep without toxin')
    ...
    predicted_set = PeptideDataset(data=raw_data)
    ...
    df = pd.DataFrame({
        'Sequence': peptide,
        'Toxin Labels': predictions_labels,
        'Toxin Probabilities': predictions_probs
    })

    df_merge = pd.merge(raw_data, df, on='Sequence')
    df_extracted = df_merge[df_merge['Toxin Labels'] == 0]


    df_extracted.sort_values(by='Toxin Probabilities', inplace=True, ascending=False)

    df_merge.to_csv(output_path, index=None)
    df_extracted.to_csv(candidate_pep_path, index=None)
